...
Поток:
  ...
  4. Добавляет котировки золота и серебра (metals.dev) в конец
  5. Шлёт в Telegram-канал, сохраняет журналы (sent.json, recent_digests.json) и архив (digests/)

Окно сбора: по будням 36ч, по понедельникам 72ч (захват пятницы-вечера и выходных).
Запуск по будням; в сб/вс не работает.

Секреты (GitHub Actions Secrets):
  TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID  — токен бота и id боевого канала
  AI_API_KEY, AI_BASE_URL, AI_MODEL     — Gemini
  METALS_API_KEY                        — ключ metals.dev для котировок
"""
...
AI_SYSTEM = (
    ...
    "Рыночную аналитику оставляй ТОЛЬКО при наличии конкретного нового факта: пересмотр прогноза крупным банком, "
    "свежие макроданные, значимое движение цен, изменение спроса/предложения или важный отраслевой вывод. "
    "Частные мнения мелких аналитиков без нового факта — понижай в приоритете или отсекай.\n\n"
    "СКЛЕЙКА ДУБЛЕЙ (ВАЖНО): одно и то же событие часто приходит из разных источников (ТГ и сайты) и с РАЗНЫМИ "
    "формулировками, деталями и даже цифрами. Считай новости ДУБЛЯМИ, если они об ОДНОМ И ТОМ ЖЕ событии — "
    "та же компания и та же суть (например «Полюс объявил дивиденды за I квартал», даже если в одном источнике "
    "39,5 млрд, а в другом 28 млрд, или разные формулировки). ОБЪЕДИНЯЙ такие в ОДИН пункт (бери самые точные/полные "
    "цифры), НИКОГДА не давай два пункта про одно событие.\n"
    "АНТИ-ПОВТОР ПО ДНЯМ: если тема уже была в прошлых дайджестах (список ниже) — НЕ повторяй её, даже если сейчас "
    "она пришла в другой формулировке или с другими цифрами, КРОМЕ случая, когда есть реально НОВОЕ развитие события.\n\n"
    "ОЦЕНКА ВАЖНОСТИ И ПРИОРИТИЗАЦИЯ: аудитория дайджеста — сотрудники и менеджмент отраслевых компаний "
    "по золоту и серебру: менеджеры, аналитики, сейлзы, руководители направлений и потенциально топ-менеджмент. "
    "Отбирай новости, которые помогают понять рынок, регуляторику, добычу, проекты, спрос/предложение, "
    "конкурентов и отраслевые риски.\n"
    "ПРИОРИТЕТЫ при отборе и сортировке:\n"
    "1) Системные рыночные события: сильные движения цен золота/серебра, прогнозы крупных банков, решения ФРС/ЦБ, "
    "ставки, доллар, инфляция, спрос центробанков, ETF, физический спрос, дефицит/профицит.\n"
    "2) Россия, СНГ и близкие рынки: Минфин, ЦБ, Минприроды, Роснедра, налоги, лицензии, аукционы, "
    "экспортные ограничения, геологоразведка, добыча, переработка, инфраструктура.\n"
    "3) Крупные производители и значимые проекты: Полиметалл, Полюс, Селигдар, ЮГК, Highland Gold и сопоставимые "
    "российские/международные игроки. Выше ставь новости о добыче, запасах, CAPEX, запуске/остановке рудников, "
    "M&A, дивидендах и крупных проектах.\n"
    "4) Серебро не теряй: если есть значимые новости по серебру, включай 1-2 пункта, если они не слабее остальных. "
    "Учитывай серебро как отдельный рынок, часть полиметаллических проектов и сырьевых цепочек.\n"
    "5) Операционные и регуляторные риски: аварии, остановки, проверки, санкции, экспорт/импорт, налоги, лицензии "
    "и правила недропользования повышай в приоритете, если они влияют на добычу, поставки, себестоимость или сроки.\n"
    "6) Мелкие зарубежные junior-компании, локальные PEA/FS/NPV-новости и частные назначения включай только при "
    "реально заметном масштабе: крупные запасы, CAPEX, M&A, запуск добычи или важный рыночный сигнал. "
    "Не ставь их выше российских/СНГ, рыночных и регуляторных событий только из-за красивых цифр в пресс-релизе.\n"
    "БАЛАНС ВЫПУСКА: хороший дайджест покрывает разные типы событий: рынок/цены, РФ и СНГ, добыча/проекты, "
    "регуляторика, серебро, крупные международные компании. Не набирай выпуск из 3-4 однотипных прогнозов по золоту "
    "или нескольких мелких зарубежных корпоративных новостей. Если несколько материалов об одной рыночной идее — "
    "объедини их в один сильный пункт.\n"
    "Сначала самое важное для отраслевой аудитории, затем менее важное — строго по убыванию значимости. "
    "Проходные, узкие, рекламные и слабые новости убирай, даже если они формально про золото или серебро.\n\n"
    "Пунктов: от 6 до 8. Если действительно значимого меньше 6 — дай меньше, не добивай мусором.\n"
    "НЕ ДОБИВАЙ ВЫПУСК СЛАБЫМИ НОВОСТЯМИ: если есть 5-6 сильных событий, не добавляй мелкие junior-проекты, "
    "частные прогнозы и проходные корпоративные назначения только ради 8 пунктов. Лучше меньше пунктов, "
    "но с нормальной отраслевой значимостью.\n"
    ...
    "деталь: цифры, сроки, причина, последствие или контекст для рынка. Если новость короткая и добавить "
    "нечего по существу — оставь 1 ёмкое предложение. НЕ лей воду и не растягивай ради объёма: "
    "вторая фраза нужна, только если несёт реальную информацию.\n\n"
...
# ---------- Котировки золота и серебра (metals.dev, latest) ----------
def fetch_prices():
    ...
    try:
        ...
        if gold is None and silver is None:
            print(f"[!] metals.dev: нет цен. Ответ: {str(data)[:200]}")
            return None

        cur = {"gold": gold, "silver": silver}
        ...
        if gold is not None:
            lines.append(f"<i>Au: {fmt(gold)} $/унц.{arrow('gold')}</i>")
        if silver is not None:
            lines.append(f"<i>Ag: {fmt(silver)} $/унц.{arrow('silver')}</i>")
